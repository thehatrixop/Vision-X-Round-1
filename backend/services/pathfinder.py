...
class PathfinderService:
    ...
    def get_shortest_path_by_coords(self, start_lat: float, start_lon: float, end_lat: float, end_lon: float) -> Dict[str, Any]:
        """
        Computes routing path directly between arbitrary manual GPS coordinates.
        """
        start_info = {"name": f"Custom Pin ({start_lat:.4f}, {start_lon:.4f})", "lat": start_lat, "lon": start_lon}
        end_info = {"name": f"Custom Pin ({end_lat:.4f}, {end_lon:.4f})", "lat": end_lat, "lon": end_lon}
        ...
        if not ors_result:
            ...
            ors_result = {"coordinates": coordinates, "distance_m": dist_m, "engine": "Direct GPS Pathfinder"}
